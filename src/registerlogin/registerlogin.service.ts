...
export class RegisterLoginService {
    ...
    private async getConnection(cn_str: string, dbName: string): Promise<Connection> {
        ...
        try {
            ...
            console.log(`Connected to DB: ${dbName}`, conn);
    ...
    async resolveAppConfig(appName: string): Promise<AppConfigType> {
        try {
            // Use DatabaseService to get app config
            const db = await this.databaseService.getAppDB(appName);
            if (!db) {
                throw new NotFoundException(`App config not found for ${appName}`);
            }
            ...
            // Normalize common field names to what this service expects
            ...
            if (!config.cn_str || !config.db) {
                throw new BadRequestException(`Invalid config for ${appName}: missing connection string or db name`);
            }
            return { cn_str: config.cn_str, dbName: config.db };
        } catch (err: any) {
            throw new BadRequestException(`App config not found for ${appName}: ${err.message}`);
        }
    }
    ...
    async signupUser(dto: any) {
        ...
        try {
            ...
            if (type === 'otp') {
                await this.otpService.sendOtp(email.toLowerCase(), 4, 2, appName, 2);
    ...
    // ===== LOGIN WITH OTP (ANY NUMBER/EMAIL) =====
    async loginUser(dto: any) {
        ...
        try {
            // Resolve app config to ensure app exists
            await this.resolveAppConfig(appName);
            // Send OTP to any email/mobile, no DB check
            await this.otpService.sendOtp(name, 4, 2, appName, 2);
