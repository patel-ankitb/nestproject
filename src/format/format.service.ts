...
export class FormatService {
  ...
  // ===== Helper: Convert price string to full number string =====
  private convertPriceToNumber(priceStr: string | null): string {
    if (!priceStr || priceStr.toLowerCase() === 'n/a') return 'N/A';
    let numericPart = priceStr.replace(/₹/g, '').trim();
    let multiplier = 1;

    if (/lakh/i.test(numericPart)) {
      multiplier = 100000;
      numericPart = numericPart.replace(/lakh/gi, '').trim();
    } else if (/crore/i.test(numericPart)) {
      multiplier = 10000000;
      numericPart = numericPart.replace(/crore/gi, '').trim();
    }
  ...
  async getModuleData(headers: any, body: any) {
    ...
    if (moduleName === 'newvarinet') {
      ...
      for (const [modelId, variants] of Object.entries(grouped)) {
        ...
        for (const variant of variants) {
          ...
          // ===== Convert price to basedPrice and update =====
          const priceStr = variant?.sectionData?.newVariants?.price || null;
          const basedPrice = this.convertPriceToNumber(priceStr);

          await collection.updateOne(
            { _id: variant._id },
            {
              $set: {
                'sectionData.newVariants.basedPrice': basedPrice
              }
            }
          );
        }
    ...
    // ... other existing code for format updates ...
