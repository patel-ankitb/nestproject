...
export class AccessControlMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    const action = 'get-config'; // Hardcoded for this endpoint
    ...
    try {
      ...
      if (!role) {
        throw new HttpException('Role not found.', HttpStatus.FORBIDDEN);
      }
      ...
      if (!moduleAccess) {
        throw new HttpException('Module access denied.', HttpStatus.FORBIDDEN);
      }
